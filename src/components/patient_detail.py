...
#     logger.debug(f"Detection Sessions: {detection_sessions}")
...
#         # Date column
#         with cols[1]:
#             # Convert the date string to datetime and format to show date only
#             date_str = session['detection_date']
            
#             # Parse the datetime string and format to show only the date
...
def render_detection_sessions(patient):
    """Display patient detection sessions in a table format with image previews."""
    # Add New Detection button at the top
    ...
    # Render detection dialog
    if st.session_state['show_detection_dialog']:
        st.subheader("New Detection")
        
        # First form for image upload and detection
        ...
        # Second form for saving detection results
        if st.session_state['detection_complete']:
            with st.form(key='save_form'):
                # Show detection results
                st.json(st.session_state['detection_result'])
                
                # Additional form fields
                diagnosis = st.text_area("Diagnosis Result", key="diagnosis_result")
                followup = st.text_area("Follow-up Plan", key="followup_plan")
                ...
                if save_submitted:
                    ...
                    for file in st.session_state['uploaded_images']:
                        image_paths.append(f"local_files/images/{file.name}")
                    
                    new_session = {
                        'detection_date': datetime.now(),
                        'detection_images': image_paths,
                    ...
                    session_created = create_detection_session(
                        user_id=USER_ID,
                        patient_id=patient['id'],
                    ...
                    if session_created:
                        st.success("Detection session saved successfully!")
                        ...
                        st.error("Failed to save detection session")
